...
async def get_cities(db: AsyncIOMotorDatabase = Depends(get_database)) -> List[CityResponse]:
...
async def get_city(city_id: str, db: AsyncIOMotorDatabase = Depends(get_database)) -> CityResponse:
